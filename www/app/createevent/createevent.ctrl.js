...
angular.module('app')
  .controller('EventdetailCtrl', function($scope, $stateParams, $window){
    ...
    xhr.open("GET", $rootScope.API_URL + "/events/" + $stateParams.id + "?access_token=" + $window.localStorage['access_token']);
    ...
    xhr.open("GET", $rootScope.API_URL + "/events/" + $stateParams.id + "/accounts?access_token=" + $window.localStorage['access_token']);
    xhr.setRequestHeader("cache-control", "no-cache");
